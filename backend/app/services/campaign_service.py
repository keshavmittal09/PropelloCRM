...
async def process_campaign_row(row: dict, campaign: Campaign, db: AsyncSession, existing: Lead | None = None) -> dict:
    ...
    # Use preloaded existing lead when provided (avoids per-row DB lookup)
    ...
    if existing:
        ...
        db.add(contact)
        # no flush here; ids are generated by Python-side defaults (uuid4)
    ...
    # avoid per-row flush; commit handled by caller
